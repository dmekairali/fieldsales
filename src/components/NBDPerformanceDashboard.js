...
const NBDPerformanceDashboard = ({ mrName, dateRange: propDateRange, performanceFilter: propPerformanceFilter }) => {
    ...
    const getFilteredData = () => {
        ...
        if (sortBy === 'nbd_conversion_rate') {
            filtered = filtered.sort((a, b) => (b.nbd_conversion_rate || 0) - (a.nbd_conversion_rate || 0));
        } else if (sortBy === 'nbd_visits') {
            filtered = filtered.sort((a, b) => (b.nbd_visits || 0) - (a.nbd_visits || 0));
        } else if (sortBy === 'nbd_revenue') {
            filtered = filtered.sort((a, b) => (b.nbd_revenue || 0) - (a.nbd_revenue || 0));
        } else if (sortBy === 'nbd_visit_percentage') {
            filtered = filtered.sort((a, b) => (b.nbd_visit_percentage || 0) - (a.nbd_visit_percentage || 0));
        }
                ...
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                <option value="nbd_conversion_rate">Conversion Rate</option>
                                <option value="nbd_visits">Visit Count</option>
                                <option value="nbd_revenue">Revenue</option>
                                <option value="nbd_visit_percentage">NBD Percentage</option>
                            </select>
                        </div>
                    </div>
                </div>
